...
class App extends Component {
  ...
  removePersonnage(personnage) {
    this.setState({
      personnages: this.state.personnages.filter((el) => el.key !== personnage.key });
    }, () => {
      firebase.database().ref().update(this.state.personnages);
    });
  }
  addPersonnage(e) {
        ...
        {this.state.personnages.length > 0 && (
          <ul>
            ...
            this.state.personnages.map(personnage => (
              <li>
                {personnage.nom}
                <button onClick={this.removePersonnage(personnage)}>X</button>
              </li>
            ))}
          </ul>
        )}
