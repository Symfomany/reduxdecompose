...
class App extends Component {
  ...
  removePersonnage(key, personnage) {
    console.log(configuration.database().ref(`/key`), personnage);
    // configuration
    //   .database()
    //   .child(key)
    //   .removeValue();
      ...
      <div className="App">
        ...
        {this.state.personnages.length > 0 && (
          <ul>
            ...
            this.state.personnages.map((personnage, key) => (
              <li>
                {personnage.nom}
                <button onClick={el => this.removePersonnage(key, personnage)}>
                  X
                </button>
              </li>
            ))}
